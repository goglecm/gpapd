...
instructions are defined and described in the instruction set.
...
The instruction format can be described in terms of a number of attributes.

\subsubsection{Length}

\begin{itemize}
    \item{Fixed length - all instructions are of equal length. All instructions
        in the memory are aligned. This type of instructions take more memory to
        store. They can be decoded faster than variable length instructions as
        an entire instruction can be fed into the decoder at once.}
    \item{Variable length - instructions may have different lengths. There are
        ...
        instructions are not aligned. These instructions take less memory to
        store.}
\end{itemize}
